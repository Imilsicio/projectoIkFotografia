...
$sql = 'SELECT * FROM cliente inner join usuario  on usuario.id=cliente.idcliente ';
$statement = $connection->prepare($sql);
$statement->execute();
$cliente = $statement->fetchAll(PDO::FETCH_OBJ);


$sql = "SELECT COUNT(*) FROM cliente where status ='pendente'";
$statement = $connection->prepare($sql);
$statement->execute();
$countPendente = $statement->fetchColumn();

$sql = "SELECT COUNT(*) FROM cliente where status ='activo'";
$statement = $connection->prepare($sql);
$statement->execute();
$countActivo = $statement->fetchColumn();

$sql = "SELECT COUNT(*) FROM cliente where status ='feito'";
$statement = $connection->prepare($sql);
$statement->execute();
...
<body>
        ...
        <div class="wrapper">
            ...
            <nav id="sidebar">
                <div class="sidebar-header ">
                    <h3 class="col-md-auto"><a href="index1.php">IK Fotografia</a></h3>
                </div>
    ...
    <?php elseif ($adm == 2) : ?>
        ...
        $sql = "SELECT * FROM cliente c inner join usuario u on c.idcliente=u.id WHERE   u.id='$usuario[3]'";
        ...
        $sql = "SELECT COUNT(*) FROM cliente c inner join usuario u on c.idcliente=u.id where status ='activo' and fotografo='$usuario[1]' and u.id='$usuario[3]'";
        $statement = $connection->prepare($sql);
        $statement->execute();
        $countActivo = $statement->fetchColumn();

        $sql = "SELECT COUNT(*) FROM cliente c inner join usuario u on c.idcliente=u.id where status ='feito' and fotografo='$usuario[1]' and u.id='$usuario[3]'";
        $statement = $connection->prepare($sql);
        $statement->execute();
        $countfeito = $statement->fetchColumn();

        $sql = "SELECT COUNT(*) FROM cliente c inner join usuario u on c.idcliente=u.id where  fotografo='$usuario[1]' and u.id='$usuario[3]'";
        $statement = $connection->prepare($sql);
        $statement->execute();
        $countCliente = $statement->fetchColumn();
                                    ...
                                    <table style="text-align: center;" class="table table-hover ">
                                        ...
                                        <?php foreach ($mm as $person) : ?>
                                            <tr>
                                                ...
                                                <td><a href="dashboard.php?id=<?= $person->idcliente ?>"><?= $person->nomeUsuario ?></a></td>
                                                <td><?= $person->evento; ?></td>
